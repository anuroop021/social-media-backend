...
const login = async (req, res) => {
  try {
    ...
    logger.verbose("Login attempt:", { username, password });
    const user = await getUserByUsername(username);
    logger.verbose("User from DB:", user);
    if (!user) {
      logger.critical("Login failed: user not found");
      return res.status(401).json({ error: "Invalid credentials" });
    }
    const isValidPassword = await verifyPassword(password, user.password_hash);
    logger.verbose("Password valid:", isValidPassword);
    if (!isValidPassword) {
      logger.critical("Login failed: password mismatch", {
        input: password,
